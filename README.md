...
작은 트레이에서도 구분할 수 있도록 아이콘 상단에 `C5`·`CW`·`CF` 또는 `X5`·`XW`를 직접 표기합니다.
Claude 아이콘은 오른쪽 단자, Codex 아이콘은 왼쪽 단자를 사용합니다. Codex는 로그의 실제
`window_minutes`가 24시간 이상이면 위치와 무관하게 `XW`로 표시하므로, 주간 한도만 적용되는 기간에도
5시간 창으로 잘못 보이지 않습니다.
...
프로젝트 GitHub 저장소와 개발 블로그도 바로 열 수 있습니다.
